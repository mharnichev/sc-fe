...
const canCreateFromBase = computed(() => baseServiceOptions.value.length > 0)

const resetForm = () => {
  editing.value = null
  form.mode = canCreateFromBase.value ? 'base' : 'custom'
  form.base_service_id = ''
  form.name = ''
  form.description = null
  form.duration_minutes = form.mode === 'custom' ? 30 : null
  form.price = form.mode === 'custom' ? 0 : null
  form.is_active = true
  formError.value = ''
}
...
const validate = () => {
  if (!barberId.value) return 'Ваш акаунт не прив’язаний до профілю майстра.'
...
const deleteService = async (service: MasterService) => {
  if (!barberId.value || !confirm(`Disable service "${service.name}"? It will be removed from активний lists but kept in history.`)) return
...
watch(canCreateFromBase, value => {
  if (!editing.value && value && form.mode === 'custom') {
    form.mode = 'base'
  }
}, { immediate: true })
</script>
  ...
  <div class="space-y-6">
    ...
    <section class="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
      <form class="space-y-5 rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
        <h2 class="text-xl font-semibold text-slate-900">{{ editing ? 'Редагувати послугу' : 'Створити послугу' }}</h2>
