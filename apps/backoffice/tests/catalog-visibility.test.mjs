...
test('product filters use base selects, submit on enter, and accept a brand query', () => {
  ...
  assert.match(productsSource, /<form @submit\.prevent="applyFilters">/)
  ...
  assert.match(productsSource, /class="grid min-w-0 gap-4 md:grid-cols-2"/)
  assert.match(productsSource, /class="grid gap-3 sm:grid-cols-2 xl:flex xl:flex-wrap xl:justify-end"/)
