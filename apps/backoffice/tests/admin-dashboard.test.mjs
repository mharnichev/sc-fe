...
test('dashboard page uses the single typed business endpoint without legacy metric assembly', async () => {
  ...
  assert.match(pageSource, /const dashboardAsyncDataKey = computed/)
  assert.match(pageSource, /const reviewMetricsAsyncDataKey = computed/)
  assert.match(pageSource, /const bookingRecoveryAsyncDataKey = computed/)
  assert.match(pageSource, /useAsyncData\(\s*dashboardAsyncDataKey,/)
  assert.match(pageSource, /useAsyncData\(\s*reviewMetricsAsyncDataKey,/)
  assert.match(pageSource, /useAsyncData\(\s*bookingRecoveryAsyncDataKey,/)
