...
test('customer SMS and all master campaigns are split into recipient tables', () => {
  ...
  assert.match(smsPanelSource, /getMessagingCampaigns\(page\.value, pageSize, \{ status: statusFilter\.value, recipient: 'customer', channel: 'sms', view: 'notifications' \}\)/)
  assert.match(smsPanelSource, /getMessagingCampaigns\(page\.value, pageSize, \{ status: statusFilter\.value, recipient: 'master', view: 'notifications' \}\)/)
  assert.match(smsPanelSource, /caption: 'Telegram та email кампанії для майстрів'/)
  assert.match(smsPanelSource, /Telegram- й email-повідомлення для майстрів/)
  assert.doesNotMatch(smsPanelSource, /group\.key === 'master'[^\n]+channel="telegram"/)
