...
const props = defineProps<{ token: string, cancelMode?: boolean }>()
...
const loadActivity = async () => {
  ...
  activity.value = null
  if (!props.token) { state.value = 'expired'; return }
  state.value = 'loading'
  try { activity.value = await domain.resolveCustomerActivity(props.token); state.value = 'ready' }
...
const cancelTarget = async () => {
  ...
  try {
    if (target.kind === 'booking') await domain.cancelCustomerActivityBooking(target.item.public_id, props.token)
    else await domain.cancelCustomerActivityWaitlist(target.item.public_id, props.token)
    cancellationTarget.value = null
    await loadActivity()
...
const close = () => { if (!cancelling.value) { isOpen.value = false; emit('closed') } }
...
<template>
  <BaseModal :model-value="isOpen" :dialog-label="labels.dialog" :close-label="terms.common.close" :block-close="cancelling" type="right" @update:model-value="close">
    <div data-hj-suppress class="customer-activity-modal min-h-full bg-white px-6 pb-10 pt-14 sm:px-8 sm:pb-12 sm:pt-16">
      ...
      <div v-else-if="state === 'expired'" class="grid min-h-[18rem] place-items-center text-center"><div class="max-w-sm"><h1 class="type-page-title text-2xl uppercase leading-tight">{{ token ? labels.expiredTitle : labels.missingTitle }}</h1><p class="mt-4 text-sm leading-6 text-neutral-600">{{ token ? labels.expiredDescription : labels.missingDescription }}</p><BaseButton to="/#booking" class="mt-7">{{ labels.book }}</BaseButton></div></div>
      ...
      <template v-else-if="activity">
        <p class="type-eyebrow text-xs text-neutral-500">{{ labels.eyebrow }}</p><h1 class="type-page-title mt-3 text-3xl uppercase leading-tight sm:text-4xl">{{ labels.title }}</h1>
        ...
        <section class="mt-10" :aria-label="labels.waitlist">
          <h2 class="text-lg font-semibold">{{ labels.waitlist }}</h2>
          <p v-if="!activity.waitlist.length" class="mt-3 text-sm leading-6 text-neutral-600">{{ labels.noWaitlist }}</p>
          <ul v-else class="mt-3 grid gap-3" role="list">
