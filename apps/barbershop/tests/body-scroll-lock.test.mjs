...
test('booking price details use the base accordion', async () => {
  ...
  assert.match(booking, /<AppTransition name="booking-quote-panel" mode="default" appear>[\s\S]*?<section[\s\S]*?v-if="activeStepIndex === lastStepIndex && isContactComplete"/)
  assert.match(booking, /<Transition name="booking-quote-state" mode="out-in">/)
  assert.match(booking, /v-else-if="!quoteReview\.quote" key="checking"/)
  assert.doesNotMatch(booking, /<p v-else>\{\{ isContactComplete \? quoteCopy\.review/)
  assert.match(booking, /@change="syncAutofilledContact"/)
  assert.match(booking, /@animationstart="handleContactAutofill"/)
...
test('first-visit promotion highlights only the backend discount and uses special-offer wording', async () => {
  ...
  assert.match(notice, /booking-promotion-notice__terms/)
  assert.match(notice, /@media \(max-width: 570px\)/)
  assert.match(notice, /font-size: 10px/)
